...
def determine_parameters(state, function):
    ...
    prompt = (
        ...
        "Return parameters in the format of: {\"parameter1\": \"chosen_parameter\", \"parameter2\": \"chosen_parameter2\"}, using the correct parameter names."
        )
...
def llm_reason2(query, available_functions):
    
    print("llm_reason2 function was called.")
    print("available_functions inside of llm_reason2: " + str(available_functions))

    prompt = f"""This is the user's: {query}. Think: What is the best way to achieve this? Think step by step. Break down complex expressions into simple operations. 
Evaluate one part at a time. Action: (please provide a function call or action). The functions available to you are: {available_functions}. Return only one function name, strictly as a single word, without parameters or brackets."""
...
class Agent2():
    ...
    def run(self, input_text):
        ...
        while iteration < self.max_iteration:
            ...
            response = llm_reason2(self.state['query'], self.available_functions)  #need to pass availabke_functions to allow it to see
            print("response inside of react_loop, after llm_reason is: " + str(response))
            action = response
...
trialAgent2 = Agent2("An agent to answer user's queries", [add, get_weather, fetch_schools_from_supabase, multiply], 3)
result2 = trialAgent2.run("what is 18 + 15 * 7?")
...
#print("Testing llm_reason2 directly:")
#llm_reason2({"query": "What is 5 + 7?"}, [add, get_weather])