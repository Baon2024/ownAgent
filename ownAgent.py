...
def is_simple_math_expression(query):
    # Normalize input
    query = query.lower().strip()

    # Remove common question phrasing and punctuation
    query = re.sub(r"(what is|calculate|solve|find|evaluate|equals|=|\?)", "", query)

    # Try to safely evaluate it
    try:
        # Only allow math-safe characters
        if re.match(r'^[\d\s\+\-\*/\.\(\)]+$', query):
            eval(query)  # Optional: validate it can be evaluated
            return True
    except:
        return False
...
Evaluate one part at a time. Think: Does the query require calling a function, or can the answer be provided directly by you? Action: (please provide a function call or action). The functions available to you are: {available_functions}. Return only one function name, strictly as a single word, without parameters or brackets, or if no function is needed, return 'direct_response'."""
    ...
    if is_simple_math_expression(state['query']):
...
def assemble_answer_from_working_notes(working_notes, user_query):

    prompt = f"""Your role is to assemble an answer to the user's query, using only the working notes passed to you.
    The user's query is: {user_query}. Your working_notes to assemble the answer from are: {working_notes}
    ...
    print("response inside of assemble_answer_from_working_notes is: " + str(response.text))

    return response.text

def is_goal_achieved_new(assembled_answer, user_query):
    # Check if the result exists and is meaningful
        #use cohere llm to assess where value of 'result' sufficienty answers query
        #if so, return True, otherwise return False
    response = assess_result_new(assembled_answer, user_query)
    print("response boolean inside of is_goal_achieved is: " + str(response))
        # Check the response strictly for "Sufficient" and "Insufficient"
    if response == 'Sufficient':
        print("Goal Achieved!")
...
def assess_result_new(assembled_answer, user_query):
    


    prompt = f"You need to assess whether the result sufficiently answers the user's query. Query: {user_query}. Answer: {assembled_answer}. If it does, return: 'Sufficient'. Otherwise, return: 'Insufficient'. You should return these without punctuation, simply one-word. "
    response = co.chat(
    model="command-r",          # or "command-r-plus" / "command-r7b-12-2024" if you have access
    message=prompt,
    max_tokens=500,              # Set max output length
...
class Agent2():
    ...
    def __init__(self, description, available_functions, max_iteration=5, system_prompt = ""):
        self.max_iteration = max_iteration
        self.description = description
    ...
    def get_working_notes(self):
        ...
        return self.state['working_notes']
    ...
    def run(self, input_text):
        ...
        while iteration < self.max_iteration:
            #step 1 - ask the LLM for reasoning and action
            print(f"working_notes are: {self.state['working_notes']} ")
        ...
        #print("action is:" + str(action))
            ...
            # Step 4: Update state
                self.state["result"] = result
                self.state['working_notes'] = result
            
            #probably need a step to allow llm to assemble answer from working_notes
                assembled_answer = assemble_answer_from_working_notes(self.state['working_notes'], self.state['query'])
                print("assembled_answer is:" + str(assembled_answer))
            
            # Check for completion
                if is_goal_achieved_new(assembled_answer, self.state['query']): #need something to do this
                    #if goal is achieved, make state['result'] equal value of assembled_answer
                    print("Task Complete:", assembled_answer)
                    self.state["working_notes"] += assembled_answer
                    ...
                    self.state['working_notes'] = f"Failed answer from iteration {iteration} was: {assembled_answer}"
...
result2 = trialAgent2.run("What is the latest UK news?")
print("returned result from trialAgent2 is: " + str(result2))
working_notes = trialAgent2.get_working_notes()
print("working notes are: " + str(working_notes))
...
#find a way to add multi-agent management - presumaby, other agents are assigned as options that can be called
#as alternative to calling a name function, or direct response

#add a system prompt, that is present in all calls